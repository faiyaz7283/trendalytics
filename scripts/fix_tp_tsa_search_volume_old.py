...
class FixTpTsaSearchVolume:
    ...
    def transform_data(self, limit):
        ...
        with tqdm(total=total, desc="Transforming", ncols=100) as pbar:
            for model in query:
                data = self._calculate_avg(model)
                self.transformed_data.append(data)
                pbar.update()

        logger.info("Transform completed")
        self._update_search_volume()
    ...
    def _update_search_volume(self):
        """
        Update search volume from transformed models.
        """
        logger.info(f"Update start...")
        ...
        if not total:
            ...
            with database.atomic(), tqdm(desc="Updating", postfix=f"batch 0/{total_batches}") as pbar:
                for idx, batch in enumerate(chunked(self.transformed_data, chunk_size)):
                    (tp
                     .insert_many(batch)
                     .on_conflict(
...
if __name__ == "__main__":
    ...
    fix_tp = FixTpTsaSearchVolume()
    limit = int(args.limit) if args.limit else None
    fix_tp.transform_data(limit)
    logger.info("Goodbye!")
